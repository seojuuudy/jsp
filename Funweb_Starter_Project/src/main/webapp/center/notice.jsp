			...
			<table id="notice">
				...
				<c:forEach var="board" items="${boardList }">
					...
					<tr onclick="location.href='notice_content.jsp?idx=${board.idx }&pageNum=<c:out value='${param.pageNum }' default='1'/>'" >
						...
						<td>${board.name }</td>
<%-- 						<td><fmt:formatDate value ="${board.date }"/></td> <!-- jstl의 fmt 라이브러리 활용한 출력 날짜 형식 변환 --> --%>
<%-- 						<td><fmt:formatDate value ="${board.date }" type="date"/></td> <!-- jstl의 fmt 라이브러리 활용한 출력 날짜 형식 변환 --> --%>
<%-- 						<td><fmt:formatDate value ="${board.date }" type="time"/></td> <!-- jstl의 fmt 라이브러리 활용한 출력 날짜 형식 변환 --> --%>
						<td><fmt:formatDate value ="${board.date }" pattern="yy-MM-dd HH:mm" /></td> <!-- jstl의 fmt 라이브러리 활용한 출력 날짜 형식 변환 -->
